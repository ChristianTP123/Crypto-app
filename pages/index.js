...
function App() {
  ...
  function add() {
    if (pageNum < 6) {
      return <button onClick={() => {setPageNum(pageNum+1)}} type="button" className="btn btn-secondary">Add 100 more</button>
    }
    else {
  ...
  return (
    <div className="App background">
      ...
      <div className="box">
        ...
        {coinList.map(coin => {
          return (
            <Coin key={coin["name"]} logo={coin["image"]} name={coin["name"]} price={coin["current_price"]} market={coin["market_cap"]} change={coin["price_change_percentage_24h"]} id={coin["id"]}/>
          )
        })}
